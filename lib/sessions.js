...
export async function computeStreak(dayKeyFn, startOfDayFn, minSecondsPerDay = 60) {
  ...
  // Look back at most 400 days to bound the loop.
  for (let i = 0; i < 400; i++) {
    ...
    if (seconds >= minSecondsPerDay) {
      streak += 1;
      cursor = new Date(dayStart.getTime() - 1); // step into previous day
    } else if (i === 0) {
      // Today has no activity yet — that's fine, don't break the streak,
      // just don't count today. Move to yesterday and keep checking.
